...
def Neural_Sim(self, input, output):
    ...
    if env == 'Windows' or True:
        base_dir = '.\\layer_record'  # 使用 Windows 风格的路径
        input_file_name = os.path.join(base_dir, 'input' + str(self.name) + '.csv')
        weight_file_name = os.path.join(base_dir, 'weight' + str(self.name) + '.csv')
        weightOld_file_name = os.path.join(base_dir, 'weightOld' + str(self.name) + '.csv')
        trace_command_path = os.path.join(base_dir, 'trace_command.bat')
        
        with open(trace_command_path, "a") as f:
            input_activity = open('.\\input_activity.csv', "a")
            weight_q = wage_quantizer.Q(self.weight, self.wl_weight)
            write_matrix_weight(weight_q.cpu().data.numpy(), weight_file_name)
            
            if len(self.weight.shape) > 2:  # Convolutional layers
                k = self.weight.shape[-1]
                padding = self.padding
                stride = self.stride  
                activity = write_matrix_activation_conv(stretch_input(input[0].cpu().data.numpy(), k, padding, stride), None, self.wl_input, input_file_name)
                ...
                if str(self.name) == 'FC2_':
                    ...
                    input_activity.write(str(activity) + ",")
            f.write(f"\"{weight_file_name}\" \"{weightOld_file_name}\" \"{input_file_name}\" {str(activity)} ")
            input_activity.close()
    else:
        print(f"ATTENSION: it is the Unknown operating system: {env}")
...
def hardware_evaluation(model,wl_weight,wl_activation,numEpoch, env):
    hook_handle_list = []
    ...
    if env=='Linux' or env=='Darwin' or True:
        ...
        # Key part that generates the log and the CPP conmments.
    ...
    if env=='Windows' or True:
        if os.path.exists('./layer_record/trace_command.bat'):
            os.remove('./layer_record/trace_command.bat')     
        with open('./layer_record/trace_command.bat', 'w') as f:
            # f.write("@echo off\n")
            command = f".\\NeuroSIM\\main {numEpoch} .\\NeuroSIM\\NetWork.csv {wl_weight} {wl_activation} "
            f.write(command)
    else:
        print(f"The system is {env}")
